...
│
├── docs/                                  ← 핵심 문서
│   ├── _데이터처리지침서_v2.0.docx        (작업 절차)
│   └── _최종마무리_보고서_v3.0.docx       (전체 작업 결과 요약)
│
...
### 7. docs/ (핵심 문서 2개)
- **_데이터처리지침서_v2.0.docx** — 향후 작업 시 따라야 할 절차
- **_최종마무리_보고서_v3.0.docx** — 전체 작업 결과 + 자료별 특이사항
...
2. `docs/_데이터처리지침서_v2.0.docx` 절차 따라 처리
3. (`_skill_groundwater_db_integration/` 폴더 별도 위치에 SKILL.md + 스크립트 보관)
4. 처리 후 `_history.csv`에 변경 이력 추가
5. 백업 폴더 만들고 진행
