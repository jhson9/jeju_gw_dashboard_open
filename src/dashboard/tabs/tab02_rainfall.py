...
def render(asos_df: pd.DataFrame, periods: dict):
    ...
    if base_date is not None and not asos_df.empty:
        ...
        # 🆕 (2026-06-06 v6) 년/월 선택 위젯
        # 🆕 (2026-06-06 v8 사용자 요청)
        #   - 제목과 selectbox 같은 행 배치 (좌: 제목, 우: 년도·월)
        #   - "M 슬롯/사용자 선택" prefix 제거 — 제목은 YYYY-MM 만 표시
        #   - 안내 텍스트 삭제 (간결)
        #   - 차트 height 280 → 400 (연간 차트와 비슷한 높이)
        ...
        # 차트 그리기 전 현재 선택값 읽기 (위젯은 같은 행에 렌더링)
        ...
        # 🆕 (v8) 제목 + selectbox 같은 행 (제목 4 : 년도 1 : 월 1)
        # 🆕 (v9 사용자 요청) inline font-size 22px 명시 — st.columns 안에서
        #   section-title CSS 가 visual width 영향으로 작아 보이는 현상 회피.
        #   연간 강수량 분석 제목과 동일 크기 보장 (01~04탭 통일).
        _h_title, _h_year, _h_month = st.columns([4, 1, 1])
        with _h_title:
            st.markdown(
                f'<p class="section-title" '
                f'style="margin:14px 0 4px;font-size:22px !important;'
                f'font-weight:700;line-height:1.2;">'
                f'{_label} — 일별 강수량 (mm, N={_n_days}일)</p>',
                unsafe_allow_html=True
            )
        with _h_year:
            st.selectbox(
...
def _render_partial_daily_chart(asos_df: pd.DataFrame, base_date,
    ...
    for s in config.STATIONS_ASOS:
        ...
        fig.add_trace(go.Bar(
            ...
            # 🆕 (2026-06-06 v10) 라벨 폰트 10 → 14 (연간 차트와 통일)
            # 🆕 (2026-06-06 v11) uniformtext 와 함께 작동 — size 보장됨
            # 🆕 (2026-06-07 사용자 요청) 라벨: 계열색+Arial Black → 검은색·일반 굵기 (연간 차트와 통일)
    ...
    # 🆕 (2026-06-06 v10 사용자 요청) Y축 최대값 100 고정
    #   100 초과 막대는 클리핑되지만 cliponaxis=False 라 라벨은 위에 표시됨.
    #   사용자 의도: "100 넘으면 많이 온 것, 라벨로 충분히 확인 가능"
    _y_range = ([0, 1.1] if metric == "eff" else [0, 100])
    fig.update_layout(
        barmode="group",
        # 🆕 (2026-06-06 v8 사용자 요청) 일별 차트 height 280 → 400 (연간 강수량과 비슷)
        height=400,
        margin=dict(l=10, r=10, t=30, b=30),
        plot_bgcolor="white",
        xaxis=dict(title="날짜", type="category", tickfont=dict(size=12)),
        yaxis=dict(title=y_title, range=_y_range, tickfont=dict(size=12)),
        legend=dict(orientation="h", y=1.14, font=dict(size=12)),
        # 🆕 (2026-06-06 v11 5팀 분석 결과) Plotly 의 textfont 자동 축소 차단.
        #   31일 × 4지점 = 124개 막대가 좁은 폭에 배치되면 Plotly 가 텍스트를
        #   자동 축소함 → textfont.size=14 가 무시됨. uniformtext 가 핵심 해결책.
        #   minsize=12 미만이면 mode="show" 로 겹쳐도 강제 표시 (가독성 우선).
        uniformtext=dict(minsize=12, mode="show"),
        # 🆕 (v11) 막대 사이 간격 축소 — 4지점 그룹 내 막대를 더 두껍게
        bargap=0.18,
        bargroupgap=0.05,
    )
