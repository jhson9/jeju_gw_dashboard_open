...
def render() -> None:
    ...
    with _t:
        st.markdown('<p class="tab-title" style="margin:0;">43.시설재배현황</p>',
                    unsafe_allow_html=True)
    with _q:
        quit_button("quit_in_tab43")
...
def _render_polygon_distribution(yr: pd.DataFrame) -> None:
    ...
    # 읍·면·동 경계 (진하게)
    try:
        eup_gj = json.loads(_EUP_GEOJSON_PATH.read_text(encoding="utf-8"))
    ...
    def _to_wgs84_pts(_gj: dict) -> "pd.DataFrame":
        """landcover geojson → 폴리곤 중심점 lat/lon DataFrame.
        CRS 자동 감지(EPSG:3857/900913/5179) 후 4326 reproject.
        """
        # CRS 감지
        src_crs_name = ""
        ...
        with col:
            ...
            try:
                ...
                if not gj_path.exists():
                    ...
                    continue
                _gj = json.loads(gj_path.read_text(encoding="utf-8"))
                pts = _to_wgs84_pts(_gj)
            except Exception as e:
            ...
            fig.update_layout(
                ...
                showlegend=False,
                # 좌·우 두 지도 viewport 잠금 (사용자가 우연히 pan/zoom 해도
                # 좌·우 일치 깨지지 않도록) — 2026-06-01 좌우 불일치 fix
                dragmode=False,
                uirevision="tab43_poly_locked",
                mapbox=dict(
                    # 외부 타일 미사용 (17차 사용자 요청)
                    style="white-bg",
                    center=dict(lat=33.38, lon=126.55),
                    # 9.5 → 9.0 : 컬럼 폭 변화에도 제주도 전체 잘리지 않도록 여유 확보
                    zoom=9.0, layers=mb_layers,
                ),
            )
            n_pts = 0 if pts.empty else len(pts)
            st.plotly_chart(fig, use_container_width=True,
                            key=f"tab43_poly_map_{side}_{y_sel}",
                            # plotly 컨트롤 숨김 + 스크롤 줌 잠금
