# -*- coding: utf-8 -*-
# =============================================================================
# SOURCE  : jeju_groundwater_dashboard/src/dashboard/tabs/tab32_drone_2d.py
# PORTED  : 2026-06-01 (자율 작업)
# RENAMED : tab32_drone_2d.py -> tab11_drone_2d.py
...
def render() -> None:
    ...
    with _t:
        st.markdown(
            '<p class="tab-title" style="margin:0;">32.정사영상 분석</p>',
    ...
    with st.spinner("정사사진 미리보기 준비 중…"):
        preview_p = get_or_make_preview(m)
    if preview_p is None:
        st.warning(f"정사사진 미리보기 생성 실패: {m.output_path('tiles_2d')}")
        return
    ...
    folium.TileLayer(
        ...
        min_zoom=8, max_zoom=22, min_native_zoom=10, max_native_zoom=14,
        show=True,
    ).add_to(fmap)
    folium.TileLayer(
