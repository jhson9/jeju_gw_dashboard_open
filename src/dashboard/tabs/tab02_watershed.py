# ==============================================================================
#  파일명: src/dashboard/tabs/tab02_watershed.py
...
def render(
    ...
    gwlevel_diff_dict : dict, optional
        app.py 가 계산한 ``compute_gwlevel_diff_dict`` 결과.
        주어지면 (Tab01 과 동일한 단일 진실 원천) 카드의 baseline 평균을
        여기서 lookup 한다. 미주어지면 기존 인라인 계산(폴백)으로 동작 —
        하위 호환 보장.
    ...
    for i, (pk, p) in enumerate(zip(ps_keys, ps)):
        ...
        if has_ws:
            ...
            # 있으면 거기서 조회 (Tab01 과 동일 값 보장). 없으면 인라인 계산.
        ...
        html = (
            ...
            f'</p>'
            # 직전 5년 같은 달의 월합 평균 + 기간 레이블 + 편차
            f'<p style="margin:2px 0 0;">'
            f'<span style="font-size:15px;font-weight:500;color:var(--color-text-secondary);">{rv_str}</span>'
            f'&nbsp;<span style="font-size:14px;color:var(--color-text-secondary);">{yr_rain_s}년 {p["month"]}월 월합 평균</span>'
