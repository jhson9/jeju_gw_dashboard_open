...
# 🆕 (2026-06-11) 로버스트-베이지안(F) 표시 + 산정방식 비교표 + 설명 PDF 버튼
...
def render(
    ...
    *,
    ...
    # 🆕 (2026-06-11) 편차 = 로버스트-베이지안(F) 대표값 (캐시 미존재 시 현행 폴백)
    yaxis_title = f"지하수위(EL) 변동 — 로버스트-베이지안 메타분석 편차 (m)"
    st.markdown(
    ...
    # 🆕 (2026-06-11) 편차 값 = 로버스트-베이지안(F, 사전계산 캐시) 우선,
    #   캐시 미존재 (유역·기간) 은 REF(현행 단순평균) 폴백. F 는 95% 신뢰구간
    #   error bar 병기 — 불확실성 노출 (검토보고서 V2 운영 권고).
    fig = go.Figure()
    _n_ref_fallback = 0
    for key in PERIOD_KEYS:
        y_vals, txt, colors = [], [], []
        ...
        for w in ws_order:
            ...
            res = rb.resolve_dev(robust_dict, rec, w, key)
            if res is None:
                v, ci = None, None
            else:
                v, ci, _n_st, _src = res
            ...
            if v is not None and ci is not None:
    ...
    # 🆕 (2026-06-11) 산정방식 명시 + REF 폴백 발생 시 사용자 인지
    _method_note = (
        f'편차 = <b>{rb.F_LABEL}(F)</b> 대표값 (Student-t 계층모형). '
        f'{rb.F_SHORT_NOTE}'
    )
    if _n_ref_fallback > 0:
        _method_note += (
            f' <span style="color:{theme.COLOR_DANGER};">'
            f'※ 사전계산 캐시 없는 {_n_ref_fallback}건은 현행 단순평균 표시 — '
            f'⚙️ scripts/precompute_robust_bayes.py 실행 권장.</span>'
        )
    st.markdown(
        f'<div style="font-size:13px;color:var(--color-text-secondary);'
        f'margin:-6px 0 2px;">{_method_note}</div>'
        '<div style="font-size:14px;color:#888;margin:0 0 8px;text-align:right;">'
        '출처: 제주특별자치도 지하수정보관리시스템(water.jeju.go.kr)</div>',
        unsafe_allow_html=True
    )

    # 요약 — M 기간 기준으로 계산 (종합 요약이므로 최신 기간만 표시)
    # 🆕 (2026-06-11) F 우선 merged 편차로 집계 (차트와 동일 값 보장)
    m_key = "M"
    m_devs = []
    for w, r in diff_table.items():
        res = rb.resolve_dev(robust_dict, r.get(m_key), w, m_key)
        if res is not None:
            m_devs.append(res[0])
    if m_devs:
        n_above = sum(1 for v in m_devs if v > 0)
        n_below = sum(1 for v in m_devs if v < 0)
        theme.render_note_box(
            f"💡 <strong>{periods['M']['label']} 기준 {len(m_devs)}개 유역 분석 결과</strong>: "
            f"직전 {n_gw}년 평균 대비 (로버스트-베이지안 메타분석) "
    ...
    _render_watershed_details(
        ...
        robust_dict=robust_dict,
    )

    # 🆕 (2026-06-11 사용자 요청) 하단 — 산정방식 비교표 (현행 REF + A~F)
    #   + 로버스트-베이지안 설명 PDF chip 버튼 (pdf_server :8766 새 탭)
    st.divider()
    rb.render_method_comparison(robust_dict, periods, key_prefix="t1")
...
def _render_watershed_details(
    ...
    for i, (pk, p) in enumerate(zip(ps_keys, ps)):
        ...
        ssot = None  # 🆕 (2026-06-11) has_ws=False 시 NameError 가드
        if has_asos:
        ...
        # 🆕 (2026-06-11) 지하수위 편차 = 로버스트-베이지안(F) 우선.
        #   캐시 미존재 시 현행(실측−평균) 폴백 + "현행" 미니 배지.
        gw_dev_html = ""
        _fb_rec = ssot if ssot is not None else (
            {"편차": round(gw_a - gw_v, 2)}
            if (gw_a is not None and gw_v is not None) else None)
        _res = rb.resolve_dev(robust_dict, _fb_rec, sel, pk)
        if _res is not None:
            _dev, _ci, _nst, _src = _res
            gw_dev_html = (rb.dev_color_html(_dev, 2, "m", bold=False)
                           + rb.src_badge_html(_src))
        ...
        html = (
            ...
            f'&nbsp;<span style="font-size:14px;color:var(--color-text-secondary);">|</span>'
            f'&nbsp;<span style="font-size:14px;">{gw_dev_html}</span>'
            f'</p>'
            f'</div>'
            f'</div>'
            f'</div>'
    ...
    # ── 표 B: 유역 지하수위 현황 ──────────────────────────────
    st.markdown(
        f'<p class="section-title">지하수위(EL) 현황 : {sel}유역 — 로버스트-베이지안 메타분석</p>',
        unsafe_allow_html=True
    )
    st.markdown("<div style='height:4px;'></div>", unsafe_allow_html=True)
    if has_ws:
        _render_gw_table(ws_df, periods, ps_keys,
                         sel=sel, robust_dict=robust_dict)
...
def _render_gw_table(ws_df, periods, ps_keys, sel=None, robust_dict=None):
    """기간=행, 지표=열 구조.
    열: 기간 | 최근 월 수위 (m) | 과거 N년 해당월 평균 (m) | 지하수위 편차 (m)
    table-layout:fixed 로 열 폭을 균등 분배.
    🆕 (2026-06-11) 편차 = 로버스트-베이지안(F) 우선 — 캐시 미존재 시 현행
    (실측−평균) 폴백 + "현행" 미니 배지."""
    ps = [periods[k] for k in ps_keys]
    ...
    for pk, p in zip(ps_keys, ps):
        ...
        # 🆕 (2026-06-11) F 우선 편차 + 출처 배지
        _res = rb.resolve_dev(robust_dict,
                              {"편차": diff} if diff is not None else None,
                              sel, pk) if sel else None
        if _res is not None:
            _dev, _ci, _nst, _src = _res
            d_s = (rb.dev_color_html(_dev, 2, "", bold=False)
                   + rb.src_badge_html(_src))
        else:
