...
C_AGRIX = "#7E3A8A"     # AgriX 등록정보 보조지표 (2026-06-03 신설)
...
def _render_agrix_box():
    """AgriX 농작물 재배면적 보조 KPI (2026-06-03 신설).
    출처: 농림축산식품부 농업경영체 등록정보 현황 서비스 (자료갱신 2026.3.17., 추출 2026.6.3.)
    """
    ax = L.load_agrix()
    if ax is None or ax.empty or "재배면적_ha" not in ax.columns:
        return
    a = ax.dropna(subset=["재배면적_ha"]).copy()
    if a.empty:
        return
    yrs = sorted(a["연도"].unique().tolist())
    if not yrs:
        return
    y_latest = int(yrs[-1])
    ...
    def _v(year, region):
        r = a[(a["연도"] == year) & (a["시군"] == region)]
        return float(r.iloc[0]["재배면적_ha"]) if not r.empty else None

    dz_now = _v(y_latest, "도전체")
    je_now = _v(y_latest, "제주시")
    sg_now = _v(y_latest, "서귀포시")
    if dz_now is None:
        return
    yoy_txt = ""
    if y_prev is not None:
        dz_prev = _v(y_prev, "도전체")
        if dz_prev:
            pct = (dz_now - dz_prev) / dz_prev * 100
            yoy_txt = f"전년 {y_prev} {dz_prev:,.0f}ha 대비 {pct:+.1f} %"
    cards = [
        H.kpi_card(f"AgriX 재배면적 도전체 ({y_latest})", f"{dz_now:,.2f} ha",
                   yoy_txt or "작물 기준 노지·시설 합산", C_AGRIX),
    ]
    if je_now is not None:
        cards.append(H.kpi_card(f"제주시 ({y_latest})", f"{je_now:,.2f} ha",
                                (f"도내 비중 {je_now/dz_now*100:.1f} %" if dz_now else "도내 비중 —"), CITY["제주시"]))
    if sg_now is not None:
        cards.append(H.kpi_card(f"서귀포시 ({y_latest})", f"{sg_now:,.2f} ha",
                                (f"도내 비중 {sg_now/dz_now*100:.1f} %" if dz_now else "도내 비중 —"), CITY["서귀포시"]))
    H.section_title("AgriX 농작물 재배면적 비교", top=8)
    H.kpi_row(cards)
    st.caption(
        "출처: 농림축산식품부 농업경영체 등록정보 현황 서비스 "
        "([uni.agrix.go.kr](https://uni.agrix.go.kr/docs7/biOlap/)) · "
        "자료갱신 2026-03-17 · 추출 2026-06-03. "
        "※ AgriX '재배면적'은 작목·작기를 반영(이모작 중복)하므로 통계연보 '경지면적'보다 클 수 있습니다."
    )
