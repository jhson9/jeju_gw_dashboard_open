...
DATA_SOURCES: dict[str, PdfDataSource] = {
    ...
    # ── 🆕 (2026-06-11 로버스트-베이지안) 지하수위 산정방법 설명자료 — tab01/03 ──
    # data/01_rain_gwlevel/Ref/ 의 "260607_로버스트 베이지안 적용(.pdf)" 등
    # 방법론 해설 PDF 를 탭 안 chip 버튼(<a target="_blank">)으로 새 탭 오픈.
    "gw_ref": PdfDataSource(
        name="gw_ref",
        data_root=config.GW_REF_DIR,
        allowed_ext=(".pdf",),
        label="지하수위 산정방법 자료",
    ),
...
class _PDFHandler(BaseHTTPRequestHandler):
    ...
    # ⚡ (2026-06-11 검증팀 S1) HTTP/1.1 keep-alive 활성화.
    # 기본값 HTTP/1.0 은 요청마다 TCP 신규 연결 — Cesium b3dm(미션당 ~1,000개),
    # Leaflet XYZ 타일, preview/DoD PNG 모두 연결 비용을 매번 지불했음.
    # 본 핸들러는 모든 응답(200/ping)에 Content-Length 를 명시하고 send_error
    # 도 자동으로 Content-Length 를 보내므로 HTTP/1.1 전환 안전.
    ...
    def do_GET(self) -> None:  # noqa: N802 — base 시그니처
        ...
        try:
            ...
            # 🛡️ (2026-06-11 재검증팀) HTTP/1.1 keep-alive 안전 — 전송 중단된
            # 커넥션 재사용 금지 (body 가 Content-Length 보다 짧게 끝난 상태).
            self.close_connection = True
            return
        except OSError as e:
            _logger.warning("[pdf_server] read failed: %s — %s", real_target, e)
            # 🛡️ (2026-06-11 재검증팀) 헤더 전송 후 read 실패 시 body 불완전 —
            # keep-alive 재사용하면 클라이언트가 타임아웃까지 대기. 커넥션 닫기.
            self.close_connection = True
