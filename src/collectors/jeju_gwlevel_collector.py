...
# 🆕 (2026-06-06 Stage 2 H3) 표준 로거 — 무음 예외 → logger.exception()
# 같은 파일 L576/611 에 이미 logger.exception 사용 → 패턴 통일.
...
try:
    ...
    _gw_cache = _st.cache_data(ttl=600, show_spinner=False, max_entries=4)
except Exception:
    def _gw_cache(fn):
        return fn
...
def get_last_date(csv_path: Path, granularity: str) -> date | None:
    ...
    try:
        ...
        # 🆕 (2026-06-06 Stage 2 H3) 무음 예외 → logger.exception (Stage 2 통일).
        # None 반환 = "이 CSV 의 last_date 알 수 없음" → 호출자가 전체 재수집 결정.
        logger.exception("get_last_date 파싱 실패: %s (granularity=%s)", csv_path, granularity)
        return None
...
def merge_and_save(new_df: pd.DataFrame, csv_path: Path,
    ...
    if csv_path.exists():
        try:
            ...
            # 🆕 (2026-06-06 Stage 2 H3) 무음 예외 → logger.exception.
            # 빈 DataFrame 로 폴백 = "기존 데이터 손상/잠금 → 신규만 저장".
            logger.exception("기존 CSV 로드 실패 — 빈 DataFrame 폴백: %s", csv_path)
